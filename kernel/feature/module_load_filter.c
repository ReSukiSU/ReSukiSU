...
// Compressed finit_module input has no ELF metadata to inspect before the
// original loader decompresses it. For ordinary file-backed preset modules,
// derive the kernel module name from <name>.ko.{gz,xz,zst}; arbitrary fd names
// fail open. Kernel module names normalize '-' in filenames to '_'.
...
// Match the ELF module's declared .modinfo name against the load-time policy.
// Any malformed ELF data, missing metadata or unknown name fails open so the
// kernel's original module loader remains authoritative.
...
static bool ksu_finit_module_flags_valid(int flags)
{
    unsigned int valid_flags = MODULE_INIT_IGNORE_MODVERSIONS | MODULE_INIT_IGNORE_VERMAGIC;

#ifdef MODULE_INIT_COMPRESSED_FILE
    valid_flags |= MODULE_INIT_COMPRESSED_FILE;
#endif

    return !((unsigned int)flags & ~valid_flags);
}
...
int ksu_handle_finit_module(int fd, int flags)
{
    ...
    if (!ksu_blocked_preset_modules[0] || !capable(CAP_SYS_MODULE) || !ksu_finit_module_flags_valid(flags))
        return KSU_MODULE_LOAD_CONTINUE;
    ...
    if (should_block) {
        pr_info("module_load_filter: skipped %.*s via finit_module\n", (int)blocked.len, blocked.name);
        return 0;
    }
